    ...
    if ($(dataCityinput).data("value") != "") {
        ...
        if (inputarry.length > 0) {
            for (var i in inputarry) {
                ...
                $(".current_city").html(inputarryname[i])
            }
        }
    }
...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        var datahotcityxz   = __LocalDataCities.category.municipality;
        // 加载热门城市和省份
        strVarCity += '<div class="view-all" id="">';
        strVarCity += '  <p class="data-title">直辖市</p>';
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (isremove != "remove") {
                ...
                $('.data-tabs ul').html('<li class="falseclass" onclick="removenode_area(this)" data-selector="tab-all" ><a href="javascript:;"><span>x</span></a></li><li data-code=' + $(con).data("code") + ' data-name=' + $(con).data("name") + ' class="active" onclick="removenode_area(this)"><a href="javascript:;"><span>' + $(con).data("name") + '</span></a></li>');
            }
            //添加内容
            strVarCity += '<ul class="clearfix">';
            if (datatype == "danxuan") {
...
function removenode_area(lithis) {
    ...
    $(lithis).removeClass('falseclass').find('span').html('<span>全部</span><em></em>')

$(function(){
 ...
 $('.commemt_box').on('click','.uploadImg img',function(){
 	...
 	$('.morecomment').on('click',function(){
 		 ...
 		 $.post('HoldComment',{'trip_id':trip,'page':commentPage},function(res){
 			res.data.length<3?$('.morecomment').hide():$('.morecomment').show();
 			if(res.status==false){
 				return false;
 			}
 ...
 $.post('HoldComment',{'trip_id':trip,'page':1},function(res){
 			res.count>3?$('.morecomment').show():$('.morecomment').hide();
 			if(res.status==false){
 				$('.nocomment').show()
 				return false;
 			}
            ...
            for(var i=0;i<daylist.length;i++){
            	...
            	var hide = ''; //大于第四天的隐藏
           		gailanStr +='<a href="#day'+i+'" ><div class="view_list '+ hide+'">'+
							'<div class="flex">'+
							'	<span class="main_color">DAY '+ (daylist[i].dayindex+1) +'</span><span class="line">'+daycity+'</span>'+
							'</div>'+
							'<div class="flex">'+
							'	<span class="date">'+ daylist[i].date.replace('.','月') +'日</span><span class="date">'+dayplay+'</span>'+
							'</div>'+
                   ...
                   if(startLng&&startLat&&nextLng&&nextLat){
                    ...
                    transfer.search(new AMap.LngLat(startLng,startLat), new AMap.LngLat(nextLng,nextLat), function(status, result) {
                      ...
                      if(result.info=='OK'){
                      	...
                      	var time = dis/80;
                      	$(_self).html('<div>汽车交通 · '+dis.toFixed(2)+'公里 · 约'+time.toFixed(1)+'小时</div>');
                       /* $(_self).text('暂无公共交通信息');*/
                      }
...
function toTripmap (){
	$('.toTripmap , .dayc_tit strong.more').on('click',function(){
		var mapInfo = [];
		var index = $(this).parents('.daycity').find('.toTripmap').attr('index');
...
function toSport (){
	...
	$('.eachlist.toSpot .eachlist_img ,.eachlist.toSpot strong.more').on('click',function(){
		...
		if(editFlag){  //如果景点编辑过则按tripId查询详情
			window.location.href="/portal/itinerary/tripDetail.html?trip="+trip+'&icity='+daylist[dayindex].this_city_index+'&iday='+daylist[dayindex].day_arr_index+'&ispot='+spotIndex; 
			return false;
		}
		
		if(curSpot.info.id==='' || floor===''){ // 数据库的景点或者用户自己录的景点
 ...
 function renderEchats(priceArr){
       ...
       $('.downPDF').on('click',function(){
               ...
               function downPdf (){
                  ...
                  if (leftHeight < pageHeight) {
                  	pdf.addImage(pageData, 'JPEG', 47, 0, imgWidth, imgHeight );
                  } else {
                  	while(leftHeight > 0) {
                  		pdf.addImage(pageData, 'JPEG', 47, position, imgWidth, imgHeight)
     ...
     var  dayListTemplate =  '{{each daylist as value i}}\
     			<div class="daylist" dayindex="{{i}}" id=day{{i}}>\
					<div class="daycity">\
						<div class="time">\
							<p><span class="dayIndex">Day{{value.dayindex+1}}</span><span class="dateTime">{{value.date.replace(".","月")+"日"}}</span></p><a class="toTripmap" href="javascript:;" index="{{i}}">地图模式</a>\
						</div>\
					{{if value.one_city}}\
					...
					{{/if}}\
						<div class="dayc_tit">\
							...
							<div class="daytip m_outer">\
								...
								<div class="listmore"><span class="more_mark"></span><strong class="fr more">[ 地图模式 ]</strong></div>\
							</div>\
							{{/if}}\
						</div>\
					</div>\
	 ...
	 var  dayListTemplate1 =  '{{each daylist as value i}}\
     			<div class="daylist" dayindex="{{i}}" id=day{{i}}>\
					<div class="daycity">\
						<div class="time">\
							<p><span class="dayIndex">Day{{i+1}}</span><span class="dateTime">{{value.date.replace(".","月")+"日"}}</span></p><a class="toTripmap" href="javascript:;" index="{{i}}">地图模式</a>\
						</div>\
					{{if value.one_city}}\
					<div class="city_trafffic">\
						{{if value.one_city!=value.two_city }}\

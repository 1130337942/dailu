...
$(function () {
    ...
    var mapFn = {
        ...
        hover_list_marker: function (location, floor_index, this_name) {
            ...
            var marker_url = floor_index.toString()==""?"qt":floor_index;
        ...
        addgo_marker: function (location, floor_index, this_name) {
            ...
            // console.log( typeof floor_index)
            var marker_url = floor_index.toString()==""?"qt":floor_index;
    ...
    var initFn = {
        initialization: function () {
            ...
            function showData(form_data, data) {
                ...
                $('.bar_city_name').attr('data-city_lat',get_first_city_data.position.lat).attr('data-city_lng',get_first_city_data.position.lng)
                ...
                function mapTabFn(i) {
                    ...
                    if (this_citydata.addgo_arry.length == 0 && this_citydata.eat_name_arry.length == 0) {
                        ...
                        $('.bar_city_name').attr('data-city_lat',city_position.lat).attr('data-city_lng', city_position.lng)
        ...
        arrangeSchedule: function () {
            $(".f_main_next").on("click", function () {
                ...
                for (var i = 0; i < dayTime_arr.length; i++) {
                    ...
                    if (timeVal_item[1] >= 1140) {
                        day_Time_item = ((1140 - timeVal_item[0]) / 60) <= 0 ? 0 : (1140 - timeVal_item[0]) / 60;
                    } else {
                    ...
                    var night_Time_item = ((timeVal_item[1] - 1140) / 60) <= 0 ? 0 : (timeVal_item[1] - 1140) / 60;
    ...
    var templateFn = {
        ...
        add_godata: function (this_data) {
            ...
            var sport_srt = '<li class="jsSport"  data-this_tag_time="' + this_data.this_tag_time + '" data-lat="' + this_data.this_lat + '" data-lng="' + this_data.this_lng + '" data-period_time="' + this_data.period_time + '" data-this_floor="' + this_data.this_floor_index + '" >\
                                <div class="_l">\
    ...
    var cityAttractions = {
        ...
        Progressbar: function (percentage, is_color) {
            ...
            $('#progressbar').LineProgressbar({
                ...
                fillBackgroundColor: isbg+'linear-gradient(right,' + color2 + ',' + color1 + ')',
                height: '10px',
                radius: '5px'
            });
    ...
    function lbsAmapFn(search_txt) {
        ...
        $('.retrieval_popup_box').show().find('input').val("")
        $('.retrieval_middle').removeAttr("data-address_lat").removeAttr("data-address_lng").removeAttr("data-address");
        ...
        var city_lat = Number($('.bar_city_name').attr('data-city_lat'));
        var city_lng = Number($('.bar_city_name').attr('data-city_lng'));
        ...
        function poiPickerReady(poiPicker) {
            ...
            poiPicker.on('poiPicked', function (poiResult) {
                ...
                if (poi.location) {
                    ...
                    $('.retrieval_middle').attr("data-address",input_address).attr("data-address_lat",poi.location.lat).attr("data-address_lng",poi.location.lng)
    ...
    function add_address_saveFn(){
        ...
        $('.retrieval_popup_box').find('.ret_save').unbind('click').on("click",function(){
            var spot_name = $('.input_name').val(); //input name
            ...
            var postData={
                city_id : $('.map_head_city .active').attr('data-cityid'),
                latitude : lat,
                longitude:lng,
                address:address,
                spot_name:spot_name,
            ...
            $.post('NewSpot',postData,function(res){
                ...
                if(res.status){
                    ...
                    $('.new_spot_box').css({position:"absolute"}).animate({
                        ...
                        cityAttractions.addflyer('.new_spot_box', $('.address_cover'),'add_newAddress');
                        $('.address_cover').hide();
                    });
                    var addgo_obj = {}
                    ...
                    mapFn.addgo_marker(latlng_obj, "qt", spot_name);
                    $('.retrieval_popup_box').hide();

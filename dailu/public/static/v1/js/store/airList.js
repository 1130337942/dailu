$(function(){
	...
	$('.airList ul').on('click','.list_top',function(){
		...
		var query = {};
			...
			$(_self).parents('.list').toggleClass('active');
	...
	$('.searchwap .change').on('click',function(){
		var temp = $('#leave_city').val();
		$('#leave_city').val($('#dest_city').val());
		$('#dest_city').val(temp);
	})
	...
	$('.listWap').on('click','.air_price a',function(event){
		...
		var query ={};
			...
			query.domain = data.vendors[index].domain;
			query.wrapperId = data.vendors[index].wrapperId;
			...
			query.airtype = $(this).parents('.list').find('.airtype').attr('type');
	...
	$('.longTimemask search').on('click',function(){
	...
	function searchData () {
		...
		$.post('searchFlight',query,function(res){
			var listTemplate = '{{each flight as value i}}\
									...
									<img class="flight_img" src="/static/v1/img/airlogo/{{value.carrier}}.png" alt="">\
									<div class="airname">{{value.airlines}}</div>\
									<div class="airtype" type="{{value.flightTypeFullName}}" >{{value.flightNum}} {{value.flightTypeFullName}}</div>\
									</div>\

...
/********************     景点管理  ======醉美夜色  **********************/
class NightController extends AdminBaseController
{
        //简要介绍列表显示
        public function night()
        {
            ...
            $natureData = Db::name('Night')->whereOr($where)->page($p.',30')->order('is_top desc')->select();
            $natureResult = json_decode($natureData,true);
             $count = Db::name('Night')->whereOr($where)->count();
        ...
        public function do_add_night()
        {
            if($_POST)
            {
                ...
                if(Db::name('Night')->insert($introduceData))
                {
                    $this->success('保存成功!', url('night/night'));
                }  else {
                    $this->success('保存失败!', url('night/night'));
                }
        ...
        public function edit_night()
        {
            ...
            $natureAbstureData = Db::name('Night')->where(array('id'=>$id))->find();
            //图片格式处理
        ...
        public function do_edit_night()
        {
            if($_POST)
            {
                ...
                $where['id'] = $post['id'];
                if(Db::name('Night')->where($where)->update($introduceData))
                {
                    $this->success('修改成功!', url('/spot/Night/night.html?page='.$page));
                   
                }  else {
                    $this->success('修改失败!', url('Night/night'));
                }
        ...
        //删除醉美夜色下的 简要介绍
        public function delete_night()
        {
            $param = $this->request->param();
            //单个选中删除
            if(isset($param['id']))
            {
                if(Db::name('Night')->where(array('id'=>$param['id']))->delete())
                {
                    $this->success('删除成功！');
                }
            }
            ...
            {
                $ids = $param['ids'];
                if(Db::name('Night')->where(['id' => ['in', $ids]])->delete())
                {
                    $this->success('删除成功！');
                }
            }   
        ...
        public function top()
        {
            ...
            if (isset($param['ids']) && isset($param["yes"])) {
                ...
                Db::name('Night')->where(['id' => ['in', $ids]])->update(['is_top' => 1]);
            ...
            if (isset($_POST['ids']) && isset($param["no"])) {
                ...
                Db::name('Night')->where(['id' => ['in', $ids]])->update(['is_top' => 0]);

                $this->success("取消置顶成功！", '');
            }
        }

        //醉美夜色是否发布（1-发布、0-未发布,发布时间随之改变）
        public function publish()
        {
            ...
            if (isset($param['ids']) && isset($param["yes"])) {
                ...
                Db::name('Night')->where(['id' => ['in', $ids]])->update($data);
